...
const SAVE_VERSION=6,DB_NAME="stranded-colony",DB_VERSION=1,STORE_NAME="saves",ACTIVE_SAVE_KEY="active-colony";
let gameState=null;
const ui={};
const WORK_OPTIONS=[["unassigned","Unassigned"],["shelter","Maintain Shelter"],["salvage","Salvage Wreck"],["forage","Forage for Food"],["water","Secure Water"],["survey-ridge","Survey Northern Ridge"],["investigate-ridge","Investigate Ridge Moisture"]];
...
function migrateGameState(candidate){
  if(candidate?.saveVersion===3||candidate?.saveVersion===4||candidate?.saveVersion===5){
...
async function prepareSavedState(candidate){
  ...
  if(candidate?.saveVersion===3||candidate?.saveVersion===4||candidate?.saveVersion===5)await SaveManager.writeActiveSave(migrated);
  return migrated
}
...
function resolveTurn(sourceState){
  ...
  const before={...p.resources},survivorCount=Object.keys(p.survivors).length,counts={shelter:0,salvage:0,forage:0,water:0,"survey-ridge":0,"investigate-ridge":0};
  ...
  p.resources.water=Math.max(0,p.resources.water-survivorCount*TURN_RULES.waterPerSurvivor);
...
function render(){
  ...
  ui.eventPanel.hidden=true;ui.surveyPanel.hidden=!hasState;ui.ridgeMarker.hidden=!hasState;
  ...
  if(hasState){
    ...
    Object.values(p.survivors).forEach(s=>{
      ...
      const card=document.createElement("div");card.className="survivor-card";const name=document.createElement("b"),role=document.createElement("small"),select=document.createElement("select");name.textContent=s.name;role.textContent=s.role;select.dataset.survivorId=s.id;select.setAttribute("aria-label","Assignment for "+s.name);WORK_OPTIONS.forEach(([id,label])=>{if(id==="investigate-ridge"&&gameState.scenario?.established?.regionalKnowledge?.northernRidge?.status!=="Surveyed")return;if(id==="survey-ridge"&&gameState.scenario?.established?.regionalKnowledge?.northernRidge?.status!=="Observed")return;const option=document.createElement("option");option.value=id;option.textContent=label;select.append(option)});select.value=p.assignments[s.id]||"unassigned";select.addEventListener("change",changeAssignment);if(select.value!=="unassigned")card.classList.add("assigned");card.append(name,role,select);ui.survivorList.append(card)
    });
    const counts={shelter:0,salvage:0,forage:0,water:0,"survey-ridge":0,"investigate-ridge":0};Object.values(p.assignments).forEach(id=>{if(id in counts)counts[id]++});
    ui.assignmentSummary.textContent=assigned+" of "+Object.keys(p.survivors).length+" assigned • Shelter "+counts.shelter+" • Salvage "+counts.salvage+" • Food "+counts.forage+" • Water "+counts.water+" • Ridge "+(counts["survey-ridge"]+counts["investigate-ridge"]); renderPressures(p,counts);
...
function renderRegionalSurvey(knowledge){
  ...
  ui.ridgeMarker.querySelector("small").textContent=ridge.status+(ridge.status==="Surveyed"?" • damp ground":ridge.status==="Spring Confirmed"?" • water source":" • dense vegetation");
}
