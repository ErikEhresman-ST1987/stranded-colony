...
function resolveTurn(sourceState){
  ...
  const springLineComplete=Boolean(p.projects?.[PROJECTS.springLine.id]?.complete);ui.springLineVisual.hidden=!springLineComplete;
  ...
  const opportunityDefs=[
    ["assess-food","food","Food Sources Assessed","Mara Venn","Nearby edible plants and workable soil offer possibilities. Mara can establish a small food trial using gathered seed and cleared ground.","Food Trial","Commit 2 survivor-work turns and 6 Salvage to test a dependable local food source. A successful trial would reduce reliance on finite crash supplies without creating a full farming system."],
    ["assess-power","power","Power Systems Assessed","Kei Arun","Emergency power is stable enough for essentials. Kei identified a damaged auxiliary power unit in the wreck that could be recovered and rebuilt.","Auxiliary Power Recovery","Commit 2 engineering-work turns and 10 Salvage to recover and repair the unit. If restored, it would expand what colony equipment can operate reliably."],
    ["assess-shelter","shelter","Shelter Upgrade Assessed","Elena Sato","The emergency shelter is sound enough to improve. Reinforcing its frame and adding recovered environmental panels would make it more dependable.","Shelter Reinforcement","Commit 2 construction-work turns and 8 Salvage. The upgrade would improve protection and working capacity, but competes directly with other colony projects."]
  ];
  for(const [workId,key,title,observer,message,actionName,commitment] of opportunityDefs){
    if(counts[workId]>0&&!p.flags.opportunities[key]){
      p.flags.opportunities[key]={assessed:true,turn:p.turn+1,observer,message,actionName,commitment};
...
function render(){
  ...
  if(hasState){
    ...
    if(opp.food?.assessed)items.push("<b>Food — "+opp.food.actionName+"</b><br>"+opp.food.message+" <em>"+opp.food.commitment+"</em>");
    if(opp.power?.assessed)items.push("<b>Power — "+opp.power.actionName+"</b><br>"+opp.power.message+" <em>"+opp.power.commitment+"</em>");
    if(opp.shelter?.assessed)items.push("<b>Shelter — "+opp.shelter.actionName+"</b><br>"+opp.shelter.message+" <em>"+opp.shelter.commitment+"</em>");
