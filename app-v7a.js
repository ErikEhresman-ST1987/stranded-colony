...
const SAVE_VERSION=7,DB_NAME="stranded-colony",DB_VERSION=1,STORE_NAME="saves",ACTIVE_SAVE_KEY="active-colony";
let gameState=null;
const ui={};
const WORK_OPTIONS=[["unassigned","Unassigned"],["shelter","Maintain Shelter"],["salvage","Salvage Wreck"],["forage","Forage for Food"],["water","Secure Water"],["survey-ridge","Survey Northern Ridge"],["investigate-ridge","Investigate Ridge Moisture"],["evaluate-spring","Evaluate Spring"]];
...
function migrateGameState(candidate){
  if(candidate?.saveVersion===3||candidate?.saveVersion===4||candidate?.saveVersion===5||candidate?.saveVersion===6){
...
async function prepareSavedState(candidate){
  ...
  if(candidate?.saveVersion===3||candidate?.saveVersion===4||candidate?.saveVersion===5||candidate?.saveVersion===6)await SaveManager.writeActiveSave(migrated);
  return migrated
}
...
function resolveTurn(sourceState){
  ...
  const before={...p.resources},survivorCount=Object.keys(p.survivors).length,counts={shelter:0,salvage:0,forage:0,water:0,"survey-ridge":0,"investigate-ridge":0,"evaluate-spring":0};
...
function render(){
  ...
  if(hasState){
    ...
    ui.assignmentSummary.textContent=assigned+" of "+Object.keys(p.survivors).length+" assigned • Shelter "+counts.shelter+" • Salvage "+counts.salvage+" • Food "+counts.forage+" • Water "+counts.water+" • Ridge "+(counts["survey-ridge"]+counts["investigate-ridge"]+counts["evaluate-spring"]); renderPressures(p,counts);
