...
const SAVE_VERSION=8,DB_NAME="stranded-colony",DB_VERSION=1,STORE_NAME="saves",ACTIVE_SAVE_KEY="active-colony";
let gameState=null;
const ui={};
...
function migrateGameState(candidate){
  if(candidate?.saveVersion===3||candidate?.saveVersion===4||candidate?.saveVersion===5||candidate?.saveVersion===6||candidate?.saveVersion===7){
...
async function prepareSavedState(candidate){
  ...
  if(candidate?.saveVersion===3||candidate?.saveVersion===4||candidate?.saveVersion===5||candidate?.saveVersion===6||candidate?.saveVersion===7)await SaveManager.writeActiveSave(migrated);
  return migrated
}
...
function resolveTurn(sourceState){
  ...
  const before={...p.resources},survivorCount=Object.keys(p.survivors).length,counts={shelter:0,salvage:0,forage:0,water:0,"survey-ridge":0,"investigate-ridge":0,"evaluate-spring":0,"build-spring-line":0};
  ...
  p.flags.lastTurn={turn:p.turn,delta,counts,knowledgeUpdate,projectUpdate,improvements:{waterCollector:collectorWater},capabilities:{efficientSalvage,salvageYield},event};
...
function render(){
  ...
  if(hasState){
    ...
    if(ridgeStatus==="Spring Evaluated"){
      ...
      ui.springProjectStatus.textContent=springState?.complete?"Construction complete • capability pending verification":springState?.started?"Committed • "+(springState.work||0)+" / "+spring.workRequired+" construction work":p.resources.salvage>=spring.salvageCost?"Ready to commit":"Needs "+(spring.salvageCost-p.resources.salvage)+" more Salvage";
    }
